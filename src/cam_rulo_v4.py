...
class cam_Ctrl:
    ...
    def move_color(self,direct,color):	
        ...
        if color == "blue":
            ...
            # ここでは黄色を抽出する
            LOW_COLOR = np.array([10, 75, 75])
            HIGH_COLOR = np.array([40, 255, 255])
        elif color == "black":
            # ここでは黒色を抽出する
            LOW_COLOR = np.array([0, 50, 0])
            HIGH_COLOR = np.array([255, 255, 10])
        elif color == "white":
            # ここでは白色を抽出する
            LOW_COLOR = np.array([0, 0, 10])
            HIGH_COLOR = np.array([255, 10, 255])
        else:
            # ここでは青色を抽出する
